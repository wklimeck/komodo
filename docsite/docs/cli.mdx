...
### Examples
  ...
  - `km database backup`
  - `km db restore`
  - `km --profile other-komodo set var MY_VAR my_value -y`
...
In the file, you configure multiple profiles with names and aliases, and
choose which config profile to use with `km -p <profile> ...`. Each profile can then correspond
to a different Komodo Core you would like to connect to, or different places to backup to.
...
any fields you would like to be global across all profiles can be set at the top level as well.
